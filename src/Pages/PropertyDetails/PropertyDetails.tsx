...
import { Close, FavoriteBorderOutlined, FavoriteOutlined, KeyboardArrowLeftOutlined, KeyboardArrowRightOutlined, ShareOutlined, StarRateRounded } from '@mui/icons-material';
...
import PropertyDetailsSkeleton from '../../Components/Loader/PropertyDetailsSkeleton'

const PropertyDetails = () => {
    const { id } = useParams()
    const { data, isLoading } = useGetPropertyByIdQuery(id)
    const { isWishlisted, toggleWishlist } = useWishlist(data?.isWishlisted, id || "");
                    ...
                    <>
                        <div>
                            <ul className="flex items-center gap-1">
                                ...
                                <li><Link to="/property-list" className="text-text3 hover:underline">Property List</Link></li>
                                <li><span><KeyboardArrowRightOutlined className="!text-lg text-text3" /></span></li>
                                <li><p className="text-primary max-w-[180px] text-nowrap text-ellipsis overflow-hidden">{data?.title}</p></li>
                            </ul>
                        </div>
                        {data ?
                            <div>
                                    ...
                                    <div>
                                        <div className='flex justify-between items-start gap-6'>
                                            ...
                                            <div className='flex items-center justify-end gap-3'>
                                                ...
                                                <button className='w-9 h-9 flex items-center justify-center rounded-full border border-border1 text-primary hover:text-primary hover:border-primary duration-300' onClick={toggleWishlist}>
                                                    {isWishlisted ?
                                                ...
                                                <div className='mt-6'>
                                                    ...
                                                    <div className='flex justify-between items-center'>
                                                        <div className="flex items-center gap-3">
                                                            <img
                                                                ...
                                                                className="border-2 border-border1 rounded-full min-w-12 w-12 h-12 object-cover"
                                                                alt=""
                                                            />
                                                            <div className='flex flex-col'>
                                                                <p className='font-medium'><Link to={`/user/${data.host_id._id}`} className='hover:underline' >Hosted by <span className='capitalize'>{data.host_id.first_name} {data.host_id.last_name}</span></Link></p>
                                                                <span className='text-xs text-text3'>{getHostingDuration(data.host_id.createdAt)}</span>
                                                            </div>
                                                        </div>
                                                        <button className='btn2 !px-4 !capitalize'>Message Host</button>
                                                    </div>
                                                    <hr className='border-border1 my-8' />
                                                    <div className=''>
                                                        ...
                                                        <div className='mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2'>
                                                            <div className='flex flex-col'>
                                                                {/* <span className='text-sm text-text3 font-medium'>Bedrooms</span> */}
                                                                <p className='text-text1 font-medium'>{data?.bedrooms} Bedrooms</p>
                                                            </div>
                                                            <div className='flex flex-col'>
                                                                {/* <span className='text-sm text-text3 font-medium'>Beds</span> */}
                                                                <p className='text-text1 font-medium'>{data?.beds} Beds</p>
                                                            </div>
                                                            <div className='flex flex-col'>
                                                                {/* <span className='text-sm text-text3 font-medium'>Baths</span> */}
                                                                <p className='text-text1 font-medium'>{data?.bathrooms} Bath</p>
                                                            </div>
                                                            <div className='flex flex-col'>
                                                                {/* <span className='text-sm text-text3 font-medium'>PLot size</span> */}
                                                                <p className='text-text1 font-medium'>{data.max_guests} Guests</p>
                                                            </div>
                                                        </div>
                                                        <div className='mt-6'>
                                                    ...
                                                    <div className=''>
                                                        ...
                                                        <div className='mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2'>
                                                            {data?.amenities.slice(0, showAll ? data.amenities.length : 6).map((amenity, i) => (
                                                                <p className='font-medium text-text1' key={i}>{amenity}</p>
                                                            ))}
                                                            {data?.amenities.length > 6 && (
                                                                <button
                                                                    onClick={() => setShowAll(!showAll)}
                                                                    className='text-left font-medium text1 text-primary underline col-span-2 sm:col-span-3'
                                                                >
                                                                    {showAll ? "Show less" : `Show all ${data?.amenities.length} amenities`}
                                                                </button>
                                                            )}
                            ...
                            <div className='py-10 mt-10'>
                                <h1 className='text-center text-2xl lg:text-3xl font-semibold'>Property Not Found</h1>
                                <p className='text-center text-text1 mt-5'>The property you're looking for may have been removed, temporarily unavailable, or doesn't exist.</p>
                                <div className='mt-8 text-center'>
                                    <Link to='/property-list'>
                                        <button className='btn2 !capitalize'>Go back to Properties</button>
                                    </Link>
                                </div>
                            </div>
                        }
